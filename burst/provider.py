...
from da_inc.provider import log, get_setting, set_setting
...
def generate_payload(provider, generator, filtering, verify_name=True, verify_size=True):
    """ Payload formatter to format results the way da_inc expects them
    ...
    log.debug('[%s] >>>>>> %s would send %d torrents to da_inc <<<<<<<' % (provider, provider, len(results)))
