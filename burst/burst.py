...
from da_inc.provider import append_headers, get_setting, log
...
if auto_timeout:
    ...
    if dainc_addon:
        if dainc_addon.getSetting('custom_provider_timeout_enabled') == "true":
            timeout = int(dainc_addon.getSetting('custom_provider_timeout')) - 2
        else:
            timeout = 28
        log.debug("Using timeout from da_inc: %d seconds" % (timeout))
...
def search(payload, method="general"):
    ...
    Args:
        payload (dict): Search payload from da_inc.
        method   (str): Type of search, can be ``general``, ``movie``, ``show``, ``season`` or ``anime``

    Returns:
        list: All filtered results in the format da_inc expects
    """
    ...
    if not payload['silent']:
        p_dialog.create('da_inc [COLOR FFFF6B00]Burst[/COLOR]', translation(32061))
